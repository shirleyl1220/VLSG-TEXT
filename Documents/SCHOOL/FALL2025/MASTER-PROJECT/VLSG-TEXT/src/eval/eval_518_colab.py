"""
Evaluation script for DualSceneAligner (518-dim) using SceneGraph format.

Properly converts Word2Vec SceneGraph objects to CLIP format (NO scene CLIP in nodes).
"""
...
def convert_scene_graph_to_batch(query_graph, db_graph, clip_model, device):
    """
    Convert two SceneGraph objects to 518-dim format.
    
    Features: centroid(3) + color(3) + node_CLIP(512) = 518 dims
    (Scene CLIP removed - was causing collapse!)
    """
    ...
    def get_node_features_518(graph):
        ...
        # Build node features WITHOUT scene CLIP
        for node_id in graph.nodes:
            ...
            # Concatenate: 3 + 3 + 512 = 518 (NO scene CLIP!)
    ...
    def get_edge_info(graph):
        """Get edge information from SceneGraph - GPU safe."""
        ...
        if len(edge_idx) > 0 and len(edge_idx[0]) > 0:
            ...
            # CRITICAL: Filter out invalid edges (GPU safety)
    ...
    for eval_round in tqdm(range(eval_iters), desc=f"Eval {mode}"):
        ...
        for batch_idx, t_set in enumerate(sampled_test_indices):
            ...
            if debug_count < 5:
                ...
                # Show query graph details
                query = dataset[t_set[0]]
                print(f"\n📝 QUERY DETAILS:")
                print(f"  Num nodes: {len(query.nodes)}")
                print(f"  Num edges: {len(query.edge_idx[0]) if query.edge_idx and len(query.edge_idx) > 0 else 0}")
                
                # Show query objects
                node_labels = [query.nodes[nid].label for nid in list(query.nodes.keys())[:10]]
                print(f"  Objects (first 10): {', '.join(node_labels)}")
                
                # Show query edges
                if query.edge_idx and len(query.edge_idx[0]) > 0:
                    edge_samples = []
                    for i in range(min(5, len(query.edge_idx[0]))):
                        src_idx = query.edge_idx[0][i]
                        tgt_idx = query.edge_idx[1][i]
                        src_label = list(query.nodes.keys())[src_idx] if src_idx < len(query.nodes) else "?"
                        tgt_label = list(query.nodes.keys())[tgt_idx] if tgt_idx < len(query.nodes) else "?"
                        
                        if src_idx < len(query.nodes) and tgt_idx < len(query.nodes):
                            src_label = query.nodes[list(query.nodes.keys())[src_idx]].label
                            tgt_label = query.nodes[list(query.nodes.keys())[tgt_idx]].label
                            edge_samples.append(f"{src_label} → {tgt_label}")
                    
                    print(f"  Edges (first 5): {'; '.join(edge_samples)}")
                ...
                if gt_rank and gt_rank <= 3:
                    print(f"  ✅ GOOD - Model ranking correctly!")
                elif gt_rank and gt_rank <= 5:
                    print(f"  ⚠️  OKAY - Model has some signal")
                else:
...
if __name__ == '__main__':
    ...
    # Load model with 518-dim config (NO scene CLIP in nodes!)
    dummy_clip = torch.randn(args.num_relations, 512) * 0.1
    config = {
        'node_input_dim': 518,  # ← UPDATED to 518!
        'relation_dim': 512,
        'hidden_dim': 256,
