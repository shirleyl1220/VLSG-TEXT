"""
Precompute embeddings for IMG-generated ScanScribe test set.
Works with V2 checkpoint (512D edge CLIP).
"""
...
# ── Paths ──────────────────────────────────────────────────
CHECKPOINT_PATH = '/content/drive/MyDrive/VLSG_Files/checkpoints/epoch_70_163_cliprel.pth'
CACHE_DIR       = '/content/drive/MyDrive/VLSG_Files'
...
from src.models.sgaligner.src.aligner.dual_scene_aligner_v2 import DualSceneAligner  # V2!
...
# ── Load graphs ────────────────────────────────────────────
print("\nLoading graphs...")
_3dssg_scenes = torch.load(f'{CACHE_DIR}/3dssg_graphs_518D.pt',
...
# IMG dataset
scanscribe_test_img = torch.load(f'{CACHE_DIR}/scanscribe_graphs_test_518D.pt',
                                  weights_only=False, map_location='cpu')
query_graphs = {}
for sid in tqdm(scanscribe_test_img, desc="ScanScribe IMG"):
    for tid in scanscribe_test_img[sid].keys():
        key = f"{sid}_{str(tid).zfill(5)}"
        query_graphs[key] = SceneGraph(sid, txt_id=tid, graph_type='scanscribe',
                                       graph=scanscribe_test_img[sid][tid],
                                       embedding_type='word2vec', use_attributes=True)
query_graphs = {k: v for k, v in query_graphs.items() if len(v.edge_idx[0]) >= 1}
print(f"✓ Loaded {len(_3dssg_graphs)} 3DSSG graphs")
print(f"✓ Loaded {len(query_graphs)} query graphs (IMG)")
...
scene_clip_cache = {}
all_graphs = list(query_graphs.items()) + [(sid, g) for sid, g in _3dssg_graphs.items()]
for key, g in tqdm(all_graphs, desc="Scene CLIP"):
...
}, f'{CACHE_DIR}/clip_embedding_cache_img.pt')
print("✓ Saved clip_embedding_cache_img.pt")

# ── Step 2: Load model ─────────────────────────────────────
print("\nLoading model (V2)...")
base_model = DualSceneAligner(
    node_input_dim=518,
    relation_dim=512,  # V2: 512D edge CLIP
    hidden_dim=256,
...
def build_batch_from_cache(graph, node_clip_cache, rel_clip_cache,
    ...
    # Node features: centroid(3) + color(3) + node_CLIP(512) = 518
    node_feats = []
    ...
    if len(edge_idx) > 0 and len(edge_idx[0]) > 0:
        ...
        # V2: 512D edge CLIP embeddings
        valid_indices = valid_mask.nonzero(as_tuple=True)[0].tolist()
        if hasattr(graph, 'edge_relations') and graph.edge_relations and len(valid_indices) > 0:
        ...
        text_attr = torch.zeros(0, 512)  # V2: 512D
...
# ── Step 4: Precompute DB embeddings ──────────────────────
print("\nPrecomputing 3DSSG database embeddings...")
db_emb_cache = {}
with torch.no_grad():
    for scene_id, g in tqdm(_3dssg_graphs.items(), desc="DB embeddings"):
...
torch.save(db_emb_cache, f'{CACHE_DIR}/db_emb_cache_img.pt')
print("✓ Saved db_emb_cache_img.pt")

# ── Step 5: Precompute query embeddings ───────────────────
print("\nPrecomputing query embeddings...")
query_emb_cache = {}
with torch.no_grad():
    for key, g in tqdm(query_graphs.items(), desc="Query embeddings"):
        batch = build_batch_from_cache(g, node_clip_cache, rel_clip_cache,
...
torch.save(query_emb_cache, f'{CACHE_DIR}/query_emb_cache_img.pt')
print("✓ Saved query_emb_cache_img.pt")

print("\n✅ All done!")
print(f"  DB embeddings:    {len(db_emb_cache)} scenes")
print(f"  Query embeddings: {len(query_emb_cache)} queries (IMG)")
print(f"\nFiles saved:")
print(f"  - db_emb_cache_img.pt")
print(f"  - query_emb_cache_img.pt")
print(f"  - clip_embedding_cache_img.pt")
print(f"\nNow run: python eval_cached.py --cache_suffix _img --mode top10")