"""
Simple Dataset - Uses CLIP embeddings from scene graphs (518 dims)

For use with scene graphs that have unique labels and CLIP embeddings.
"""
...
def build_node_features_with_clip(node_dict):
    """
    Build 518-dim features: centroid(3) + color(3) + CLIP(512)
    
    This is the CORRECT format for your new scene graphs with unique labels!
    """
    centroid = np.array(node_dict["centroid"], dtype=np.float32)
    color = np.array(node_dict["mean_color"], dtype=np.float32) / 255.0
    
    # Load CLIP embedding from scene graph
    clip_vec = np.array(
...
class DualSceneGraphDataset(Dataset):
    """
    Simple dataset that loads CLIP-based scene graphs (518 dims).
    """
    ...
    def __init__(
        ...
        print(f"[DATASET] Node features: 518 dims (centroid + color + CLIP)")
    ...
    def _sample_pos_or_neg(self, idx):
        """Always sample a DIFFERENT scene (negative pair)"""
    ...
    def _load_scene(self, json_path):
        ...
        # Build node features with CLIP
        feats = []
        for nid in node_ids:
            n = nodes[nid]
            feat = build_node_features_with_clip(n)  # 518 dims
            feats.append(feat)
    ...
    def __getitem__(self, idx):
        ...
        ref_idx = (idx + 1) % len(self.scene_files)  # Always next scene (wraps around)
        ref_path = self.scene_files[ref_idx]
        
        print(f"Loading: {os.path.basename(src_path)} vs {os.path.basename(ref_path)}")
