...
class DualSceneAligner(nn.Module):
    ...
    def __init__(self, node_input_dim=518, relation_dim=512,  # relation_dim kept for compatibility
                hidden_dim=256, rel_clip_matrix=None, dropout=0.1):
        super().__init__()
        ...
        # ===== No relation embedding needed - using pre-computed CLIP ===== 
        # RelationEmbedding removed, text_attr is now (E, 512) CLIP embeddings
        
        # ===== Geometric Branch =====
        self.gat_geom = MultiGAT_Edge(
            n_units=[hidden_dim, hidden_dim, hidden_dim],  # ← not 512
            n_heads=[2, 2],
            edge_dim=8,
            dropout=dropout
        )
        ...
        # Project edge embeddings (512) back to node embeddings (hidden_dim)
        # self.edge_to_node_geom = nn.Linear(512, hidden_dim)
        
        # ===== Text Branch =====
        self.gat_text = MultiGAT_Edge(
            n_units=[hidden_dim, hidden_dim, hidden_dim],  # ← not 512
            n_heads=[2, 2],
            edge_dim=512,  # CLIP relation embeddings
            dropout=dropout
        )
        self.norm_text = nn.LayerNorm(hidden_dim)
        
        # Project edge embeddings (512) back to node embeddings (hidden_dim)
        # self.edge_to_node_text = nn.Linear(512, hidden_dim)
        
        # ===== Fusion =====
        self.fusion = GatedFusion(hidden_dim)
        
        # ===== Pooling =====
        pool_type = 'mean'  # Default to mean pooling
    ...
    def encode_scene(self, node_feats, geom_edges, geom_attr, text_edges, text_attr, batch):
        
        # 1. Encode nodes
        x = self.node_encoder(node_feats)  # (N, hidden_dim)
        
        # 2. Geometric branch - GAT returns NODE embeddings (N, hidden_dim)
        g = self.gat_geom(x, geom_edges, geom_attr.float())  # Convert to float, shape (E, 8)
        g = self.norm_geom(g)
        
        # 3. Text branch - GAT returns NODE embeddings (N, hidden_dim)
        if text_edges.size(1) > 0:
            # text_attr is pre-computed 512D CLIP embeddings (E, 512)
            # NO embedding needed - just pass directly
            # IMPORTANT: Use x (encoded nodes) not g (geometric output) as input
            t = self.gat_text(x, text_edges, text_attr.float())  # text_attr is already (E, 512)
            t = self.norm_text(t)
        else:
            t = g
        
        # 4. Fusion
        fused = self.fusion(g, t)  # (N, hidden_dim)
        
        # 5. Noise during training
        if self.training:
            fused = fused + torch.randn_like(fused) * 0.01
