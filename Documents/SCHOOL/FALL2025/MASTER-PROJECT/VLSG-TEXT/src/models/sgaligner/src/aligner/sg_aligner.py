...
sgaligner_src_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(sgaligner_src_dir)
...
class MultiModalEncoder(nn.Module):
    def __init__(self, modules, rel_dim, attr_dim, hidden_units=[3, 128, 128], heads = [2, 2], emb_dim = 100, pt_out_dim = 256,
        ...
        self.structure_encoder = MultiGAT_Edge(n_units=self.hidden_units,
                                               n_heads=self.heads,
                                               edge_dim=8,
        ...
        for module in self.modules:
            if module == 'gat':
                ...
                for idx in range(batch_size):
                    ...
                    src_structure_embedding = self.structure_encoder(src_objects_rel_pose, src_edges, data_dict['tot_bow_vec_object_edge_feats'][start_edge_idx - src_edges_count - ref_edges_count : start_edge_idx - ref_edges_count])
                    ref_structure_embedding = self.structure_encoder(ref_objects_rel_pose, ref_edges, data_dict['tot_bow_vec_object_edge_feats'][start_edge_idx - ref_edges_count : start_edge_idx])
...
class MultiModalSingleScanEncoder(nn.Module):
    def __init__(self, modules, rel_dim, attr_dim, hidden_units=[3, 128, 128], heads = [2, 2], emb_dim = 100, pt_out_dim = 256,
                       dropout = 0.0, attn_dropout = 0.0, instance_norm = False):
        super(MultiModalSingleScanEncoder, self).__init__()
        self.modules = modules
        self.pt_out_dim = pt_out_dim
        self.rel_dim = rel_dim
        self.emb_dim = emb_dim
        ...
        for module in self.modules:
            if module == 'gat':
                ...
                for idx in range(batch_size):
                    ...
                    structure_embedding = self.structure_encoder(objects_rel_pose, edges, bow_vec_object_edge_feats[start_edge_idx - edges_count : start_edge_idx])
...
class MultiModalImageEncoder(nn.Module):
    def __init__(self, modules, rel_dim, attr_dim, img_feat_dim, img_out_dim, img_feat_pool = 'max',
                 hidden_units=[3, 128, 128], heads = [2, 2], emb_dim = 100, pt_out_dim = 256,
                 dropout = 0.0, attn_dropout = 0.0, instance_norm = False):
        super(MultiModalImageEncoder, self).__init__()
        self.modules = modules
        self.pt_out_dim = pt_out_dim
        self.rel_dim = rel_dim
        self.emb_dim = emb_dim
